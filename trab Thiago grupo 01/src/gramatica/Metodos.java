...
/**
 * Classe responsável pelos codigos para execução.
 * 
...
public class Metodos {
	...
	/**
	 * Cria e separa a regra de produção. Iniciando um loop até que o usuario
	 ...
	 * caso tenha, é adicionado mais um alfabeto a determinada regra,caso nao tenha ele ja
	...
	public void separarRegraProducao() {
		System.out.print("Entre com regra de produção os separando por / ( Barra )"
	...
	/**
	 * Define o ponto de partida da computação da palavra.
	 */
	...
	public void computarPalavra() {
		...
		do {
			...
			// Busca por regra de produção na palavra.
			buscaRegra = 0;
			for (int i = 0; i < computandoPalavra.length(); i++) {
				for (int j = 0; j < regra.size(); j++) {
					String primeiro = computandoPalavra.charAt(i) + "";
